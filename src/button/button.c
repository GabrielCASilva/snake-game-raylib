...
void button_draw_unselect(Button *b, int x, int y) {
  int pX = 8;
  int pY = 6;
  DrawRectangleLines(x - pX, y - pY, b->width + pX * 2, b->height + pY * 2,
                     b->color);
...
void button_draw_selected(Button *b, int x, int y) {
  int pX = 8;
  int pY = 6;
  DrawRectangle(x - pX, y - pY, b->width + pX * 2, b->height + pY * 2,
                b->color);
  DrawText(b->text, x, y, b->fontsize, BLACK);
}

void button_selected(Button *b, int isSelected) { b->isSelected = isSelected; }
